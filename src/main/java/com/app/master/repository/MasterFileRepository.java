...
public interface MasterFileRepository extends JpaRepository<MasterFile, String> {
	...
	@Query(value = "WITH RECURSIVE descendants AS ( " + "    SELECT id FROM master_files WHERE id = :folderId "
			+ "    UNION ALL " + "    SELECT mf.id FROM master_files mf "
	...
	Optional<MasterFile> findByUploadJobId(String uploadJobId);
}