// Folder Operations (static2 - adjusted endpoints)
    ...
    for (const item of pendingItems) {
        try {
            ...
            if (pendingAction === 'copy') {
                url = `/drive/${item.id}/copy?destinationDriveId=${destinationId}`;
                if (item.password) {
                    url += `&password=${encodeURIComponent(item.password)}`;
                }
                await apiCall(url, { method: 'POST' });
            } else {
                url = `/drive/${item.id}/move?newParentId=${destinationId}`;
                if (item.password) {
                    url += `&password=${encodeURIComponent(item.password)}`;
                }
