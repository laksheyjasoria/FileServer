...
function renderFiles(files) {
  ...
  const folders = files.filter(
    (f) => f.driveType === "ROOT" || f.driveType === "FOLDER",
  );
  ...
  [...folders, ...items].forEach((item) => {
    ...
    if (isFolder)
      ...
      doubleClickAction = viewable
        ? `viewFile('${eId}','${eName}')`
        : `downloadFile('${eId}','${eName}')`;
    }
  ...
  if (isProtected) {
    ...
    try {
      const response = await fetch(
        `${API_URL}/drive/${folderId}?password=${encodeURIComponent(password)}`,
        {
          headers: { Authorization: `Bearer ${jwtToken}` },
        },
      );
      if (response.status === 403) {
        safeToast("Invalid password!", "error");
        return;
      }
