...
async function loadSharedByMe() {
    ...
    if (!container) { safeToast('Container not found', 'error'); return; }
...
async function copyShareLink(token) {
    ...
    } catch { safeToast('Failed to copy', 'error'); }
}

async function deleteShare(token) {
    const confirmed = await showConfirm('Delete this share?', 'Delete Share', 'Delete', 'Cancel', 'danger');
    if (!confirmed) return;
    try {
