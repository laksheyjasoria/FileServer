...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (
      !e.target.closest("#userSearchInput") &&
      !e.target.closest("#userSearchResults")
    ) {
...
function toggleShareOptions() {
  ...
  document.getElementById("sharePasswordGroup").style.display =
    type === "PROTECTED" ? "block" : "none";
  document.getElementById("userSelectionGroup").style.display =
    type === "USER_ONLY" ? "block" : "none";
}
...
function renderSelectedUsers() {
  ...
  if (!selectedUsers.length) {
    container.innerHTML =
      '<span class="empty-chip-text">No users selected</span>';
    return;
  }
  let html = "";
...
async function createShareLink() {
  ...
  const isMulti =
    window._multiShareFileIds && window._multiShareFileIds.length > 0;
  ...
  if (shareType === "USER_ONLY") {
    ...
    payload = {
      ...(isMulti
        ? { fileIds: window._multiShareFileIds }
  ...
  try {
    ...
    const shareUrl =
      result.url ||
      `${window.location.origin}/share2.html?token=/${result.token}`;
    document.getElementById("shareLinkInput").value = shareUrl;
    document.getElementById("shareLinkExpiry").textContent = expiry
      ? new Date(expiry).toLocaleDateString()
      : "Never";
    document.getElementById("shareLinkType").textContent =
      shareType === "PUBLIC"
        ? "Public"
        : shareType === "PROTECTED"
          ? "Password Protected"
