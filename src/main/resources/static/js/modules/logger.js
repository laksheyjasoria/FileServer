...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    .getElementById("createLoggerBtn")
  ...
  document
    ...
    .addEventListener("click", createLogger);
  document
...
async function loadLoggers() {
  ...
  try {
    ...
    if (e.message !== "Unauthorized") {
      container.innerHTML =
        '<div class="empty-state">Failed to load loggers.</div>';
...
function filterLoggers() {
  const query = document
    ...
    .value.toLowerCase()
    .trim();
  filteredLoggers = query
    ? currentLoggers.filter((l) => l.name.toLowerCase().includes(query))
    : currentLoggers;
...
async function createLogger() {
  ...
  try {
    const createRes = await loggerApiCall(
      `/logger/create?name=${encodeURIComponent(name)}`,
      { method: "POST" },
    );
    if (!createRes.ok) {
...
async function toggleLogger(id, type, newValue) {
  ...
  try {
    const res = await loggerApiCall(
      `/logger/${id}?info=${info}&warn=${warn}&debug=${debug}`,
      { method: "PUT" },
    );
    if (res.ok) {
