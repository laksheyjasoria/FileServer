...
async function loadFile() {
    try {
        ...
        if (!fileResponse.ok) throw new Error(`Failed to load file content (Status: ${fileResponse.status})`);
...
function displayFile(url, contentType, filename) {
    ...
    if (contentType.startsWith('image/')) {
        ...
        container.innerHTML = `<video controls class="video-viewer" autoplay><source src="${url}" type="${contentType}">Your browser does not support video playback.</video>`;
    } else if (contentType.startsWith('audio/')) {
        container.innerHTML = `<audio controls class="audio-viewer" autoplay><source src="${url}" type="${contentType}">Your browser does not support audio playback.</audio>`;
    } else if (contentType.startsWith('text/') || ['txt', 'log', 'md', 'csv'].includes(ext)) {
        fetch(url)
            .then(res => res.text())
            .then(text => { container.innerHTML = `<div class="text-viewer">${escapeHtml(text)}</div>`; })
            .catch(() => { container.innerHTML = `<div class="text-viewer">Failed to load text content.</div>`; safeToast('Failed to load text', 'error'); });
    } else if (['js', 'java', 'py', 'html', 'css', 'json', 'xml', 'yaml', 'yml', 'sql', 'sh', 'bat'].includes(ext)) {
        fetch(url)
            .then(res => res.text())
            .then(text => { container.innerHTML = `<pre class="code-viewer">${escapeHtml(text)}</pre>`; })
            .catch(() => { container.innerHTML = `<div class="code-viewer">Failed to load code content.</div>`; safeToast('Failed to load code', 'error'); });
    } else {
        container.innerHTML = `
