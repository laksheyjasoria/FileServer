  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (searchInput) {
      ...
      searchInput.addEventListener("keydown", (e) => {
        if (e.key === "Enter") fetchUsers(0);
      });
    }
    if (prevBtn) {
      prevBtn.addEventListener("click", () => {
        if (currentPage > 0) fetchUsers(currentPage - 1);
      });
    }
    if (nextBtn) {
      nextBtn.addEventListener("click", () => {
        if (currentPage < totalPages - 1) fetchUsers(currentPage + 1);
      });
    }
    ...
    async function performAction(id, type, payload) {
      try {
        ...
        if (!response || !response.ok) {
          ...
          throw new Error(
            text || `HTTP ${response ? response.status : "unknown"}`,
          );
        }
        ...
        if (!userTableBody) {
          ...
          if (userContainer) {
            ...
            errorDiv.textContent =
              "Error: Table body not found. Please refresh.";
