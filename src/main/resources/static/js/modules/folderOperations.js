...
async function createFolder() {
    ...
    try {
        const response = await apiCall('/resources/action', {
            method: 'POST',
            body: JSON.stringify({ action: 'CREATE_FOLDER', ids: [], destination: currentFolderId || null, name })
        });
...
async function executeRename() {
    ...
    try {
        const response = await apiCall('/resources/action', {
            method: 'POST',
            body: JSON.stringify({ ids: [contextMenuItem.id], action: 'RENAME', name: newName })
        });
