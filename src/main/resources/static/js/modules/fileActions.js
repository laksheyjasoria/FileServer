...
function viewFile(fileId, filename) {
  window.open(
    `${API_URL}/viewer.html?id=${fileId}&token=${jwtToken}`,
    "_blank",
  );
}
...
async function selectDestination() {
  const folders = allFiles.filter(
    (f) =>
      (f.driveType === "ROOT" || f.driveType === "FOLDER") &&
  ...
  document.getElementById("moveModalTitle").innerText =
    pendingAction === "copy" ? "Copy to" : "Move to";
  showModal("moveModal");
}
...
async function executeMove() {
  ...
  if (successCount > 0) {
    ...
    safeToast(
      `Failed to ${completedAction} ${failCount} item(s). Please try again.`,
      "error",
    );
  }
}
