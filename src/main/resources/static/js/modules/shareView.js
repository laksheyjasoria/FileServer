...
async function loadCurrentFolder() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Failed to load folder contents (Status: ${response.status})`);
...
function toggleSelectAllShared() {
    ...
    allItems.forEach(item => {
        ...
        if (cb) {
            ...
            if (newState) { selectedSharedItems.add(id); item.classList.add('selected'); }
