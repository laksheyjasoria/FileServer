...
function updateUploadStatus(uploadId, text, isFinal) {
    ...
    if (statusSpan) {
        ...
        if (isFinal) {
            ...
            else if (text.includes('Failed') || text.includes('Error')) statusSpan.className = 'failed-status';
            else if (text.includes('Cancelled')) statusSpan.className = 'cancelled-status';
        }
    }
}
